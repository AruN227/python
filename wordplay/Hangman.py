...
def get_num_attempts():
    while True:
        num_attempts = input('How many attempts do you want? [1-20] ')
        try:
            num_attempts = int(num_attempts)
            if 1 <= num_attempts <= 20:
                return num_attempts
            else:
                print('{0} is not bw 0 to 20'.format(num_attempts))
        except ValueError:
            print('{0} is not integer bw 0 to 20'.format(num_attempts))


def get_min_word_length():
    while True:
        min_word_length = input('What min word length do you want? [4-12]')
        try:
            min_word_length = int(min_word_length)
            if 4 <= min_word_length <= 12:
